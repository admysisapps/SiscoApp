...
interface AsambleaDetalleHeaderProps {
  asamblea: Asamblea;
  onShowToast?: (message: string, type: "error" | "success") => void;
}

const AsambleaDetalleHeader: React.FC<AsambleaDetalleHeaderProps> = ({
  asamblea,
  onShowToast,
}) => {
  // Función para abrir enlace con manejo de errores
  ...
  const quorumRequeridoPorcentaje = Math.round(asamblea.quorum_requerido * 100);
  const quorumAlcanzadoPorcentaje = Math.round(asamblea.quorum_alcanzado * 100);
    ...
    <>
      ...
      <View style={styles.detallesCard}>
        ...
        <View style={styles.detalleRow}>
          ...
          <View style={styles.detalleTextContainer}>
            <Text style={styles.detalleLabel}>Quórum</Text>
            <Text style={styles.detalleValor}>
              {quorumAlcanzadoPorcentaje}% de {quorumRequeridoPorcentaje}%
              requerido
            </Text>
            <View style={styles.quorumBarContainer}>
              <View
                style={[
                  styles.quorumBar,
                  {
                    width: `${Math.min(100, (quorumAlcanzadoPorcentaje / quorumRequeridoPorcentaje) * 100)}%`,
                    backgroundColor:
                      quorumAlcanzadoPorcentaje >= quorumRequeridoPorcentaje
...
export default AsambleaDetalleHeader;
